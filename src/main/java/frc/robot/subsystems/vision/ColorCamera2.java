...
public class ColorCamera2 {
    ...
    // "Sensor" readings
    private ColorCamFrame mostRecentFrame = new ColorCamFrame();


    public ColorCamera2(String name, Translation3d camLocation_robotFrame, Rotation3d camOrientation_robotFrame) {
        this.cam = new FotonCamera(name);
    ...
    public ColorCamera2(String name, Pose3d camPose_robotFrame) {
        this(name, camPose_robotFrame.getTranslation(), camPose_robotFrame.getRotation());
    }
    public ColorCamera2(String name, Transform3d camPose_robotFrame) {
        this(name, camPose_robotFrame.getTranslation(), camPose_robotFrame.getRotation());
    }
    ...
    public Optional<Translation3d> getClosestValidGamepiece() {
        ...
        return this.mostRecentFrame.closestValidGamepiece_fieldCoords;
    }
    ...
    public void periodic(SwerveDrivePoseEstimator fusedPoseEstimator) {
        ...
        this.mostRecentFrame = new ColorCamFrame();


        // Get the most recent frame from the camera,
        // and find where the robot was when that frame was captured.
        PhotonPipelineResult mostRecentResult = cam.getLatestResult();
        this.mostRecentFrame.timestampSeconds = mostRecentResult.getTimestampSeconds();
        Optional<Pose2d> interpolatedRobotPose = fusedPoseEstimator.sampleAt(this.mostRecentFrame.timestampSeconds);
        ...
        for (PhotonTrackedTarget target : mostRecentResult.targets) {
            ...
            this.mostRecentFrame.gamepieceLocations_robotCoords.add(gamepieceLocation_robotCoords);
            ...
            if (!FlyingCircuitUtils.isInField(gamepieceLocation_fieldCoords, inFieldToleranceMeters)) {
                this.mostRecentFrame.invalidGamepieceLocations_fieldCoords.add(gamepieceLocation_fieldCoords);
                continue;
            }
            ...
            this.mostRecentFrame.validGamepieceLocations_fieldCoords.add(gamepieceLocation_fieldCoords);
            if (this.mostRecentFrame.closestValidGamepiece_fieldCoords.isEmpty() || (metersFromRobot < this.mostRecentFrame.metersToClosestGamepiece)) {
                this.mostRecentFrame.closestValidGamepiece_fieldCoords = Optional.of(gamepieceLocation_fieldCoords);
                this.mostRecentFrame.metersToClosestGamepiece = metersFromRobot;
            }
        }

        // Logging, then we're done!
        Logger.processInputs(cam.getName()+"/mostRecentFrame", this.mostRecentFrame);
    }
